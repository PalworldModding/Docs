...
When writing a Lua mod, you can call a method inside a ModActor blueprint. This is useful, as your Lua mod can hook into functions that aren't exposed via Blueprints, then you can create most of your mod's functionality within Blueprints. If you haven't yet, follow the steps to make a basic Blueprint mod in the [Introduction](/docs/developers/ue4ss-modding/logic-mods/introduction.md) section, and then come back to this section.
...
Now we'll connect the PostBeginPlay event to this function. As mentioned in the [Introduction](/docs/developers/ue4ss-modding/logic-mods/introduction.md) section, PreBeginPlay is called too early to initialize our mod, so we'll use PostBeginPlay instead.
