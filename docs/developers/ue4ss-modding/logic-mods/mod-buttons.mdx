...
![ModButtonsArray](./assets/ModButtons/ModButtonsArray.png)
  - Create a new custom event called `ModMenuButtonPressed`

  ![ModMenuButtonPressed](./assets/ModButtons/ModMenuButtonPressed.png)

  - Under the newly created event, Add a input variable that is a `Integer` named `Index` ![ModMenuInt](./assets/ModButtons/MonMenuButtonPRessedInt.png)
  ...
  - Next add a `Switch on Int` action, connected to the `ModMenuButtonPressed` event.
    ...
    - Your pin's must match up to `ModButtons String Array` ![ModButtonSwitchOnInt](./assets/ModButtons/ModSwitchOnEvent.png)
...
![Example](./assets/ModButtons/example.png)